...
const PatientSection = () => {
  ...
  return (
    <div>
      <Box mb="50">
        <Box fontSize="xl" px={[0, 0, 6, 6, 6]} fontWeight="semibold" color="gray.500" mt="4">
        ...
        <Heading fontSize="3xl" px={[0, 0, 6, 6, 6]} fontWeight="semibold" color="#170041" mt="4">An Appointment at The Click of a Button</Heading>
        {listData
          .map((data) => (
            <Box key={data} fontSize="xl" px={[0, 0, 6, 6, 6]} fontWeight="semibold" color="gray.500" mt="4">{data}</Box>
          ))}
      </Box>
    </div>
  );
