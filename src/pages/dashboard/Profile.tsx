  ...
  return (
    <SpacedContainer py={6}>
      ...
      <Flex wrap="wrap" py={12} alignItems="flex-start">
        <Image src={doctorProfile?.profilePhoto} width="12rem" fit="contain" mr={6} />
        <Box>
          <Heading as="h3" fontSize="3xl" fontWeight={700} color="primary.dark">{doctorProfile?.fullName}</Heading>
          <Heading as="h4" fontSize="xl" fontWeight={400} color="brand.darkPink" mb={3}>{doctorProfile?.qualificationSuffix}</Heading>
          <Box color="gray.500" mb={3}>
            No reviews yet.
          </Box>
          <Text fontSize="lg" color="gray.500">
            {doctorProfile?.professionalBio}
          </Text>
        </Box>
      </Flex>
    </SpacedContainer>
